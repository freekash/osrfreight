...
class CTH_Payment_Payfast{
	...
	function __construct(){
		...
		$this->payfast_url = easybook_addons_get_option('payments_test_mode') == 'yes'? 'https://sandbox.payfast.co.za/eng/process/?' : 'https://www.payfast.co.za/eng/process/?';
	...
	function processBuyNow($args = array()){
        ...
        if(easybook_addons_get_option('email_confirmation') == 'yes' && easybook_addons_get_option('confirmation_address') != ''){
            $payfast_args['email_confirmation'] = 1;
            $payfast_args['confirmation_address'] = easybook_addons_get_option('confirmation_address');
        }
        ...
        // $data['amount'] = number_format( sprintf( "%.2f", easybook_addons_get_currency_rate('ZAR') * $data['amount'] ), 2, '.', '' );
        $data['amount'] = number_format( sprintf( "%.2f", (float)easybook_addons_get_option('payfast_rate') * (float)$data['amount'] ), 2, '.', '' );
        ...
        foreach( $data as $key => $val )
        {
            if(!empty($val))
            {
                ...
                if(in_array($key, array('cancel_url','return_url','notify_url')))
                    $val = str_replace("http://localhost:8888/easybook", 'https://easybook.cththemes.com', $val);
        ...
        $passPhrase = easybook_addons_get_option('payfast_passphrase');
        if( $passPhrase != '' )
        {
  ...
  //       $payfast_redirect .= $getString;
	...
	function validateIPN() {
        ...
        flush();
        // step 1
        $pfHost = easybook_addons_get_option('payments_test_mode')=='yes' ? 'sandbox.payfast.co.za' : 'www.payfast.co.za';
