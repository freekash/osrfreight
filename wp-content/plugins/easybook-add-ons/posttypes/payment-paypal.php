...
class CTH_Payment_Paypal{
	...
	function __construct($cmd = '_xclick') {
		...
		$this->paypal_url = (easybook_addons_get_option('payments_test_mode') == 'yes')? 'https://www.sandbox.paypal.com/cgi-bin/webscr/?' : 'https://www.paypal.com/cgi-bin/webscr/?';
	...
	function processBuyNow($args = array()){
		...
		$paypal_args = array(
            ...
            // easybook custom $inserted_posts_text .'|'. $current_user->ID .'|'. $current_user->user_email .'|renew_no',
	...
	function extractPaymentData(){
		...
		if($this->validateIPN()){
			...
			*/
			...
			$custom 					= urldecode( $paypal_data['custom'] );//order_id|listing_id|user_id|user_email|renew_yes
			$custom 					= explode("|", $custom);
			$order_ids 					= isset($custom[0])? $custom[0] : '';
            $order_ids                  = explode("-", $order_ids);
			// $listing_id 					= isset($custom[1])? $custom[1] : '';
