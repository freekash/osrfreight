...
abstract class ActionScheduler_Logger {
	...
	public function log_stored_action( $action_id ) {
		$this->log( $action_id, __( 'action created', 'easybook-add-ons' ) );
	}

	public function log_canceled_action( $action_id ) {
		$this->log( $action_id, __( 'action canceled', 'easybook-add-ons' ) );
	}

	public function log_started_action( $action_id ) {
		$this->log( $action_id, __( 'action started', 'easybook-add-ons' ) );
	}

	public function log_completed_action( $action_id ) {
		$this->log( $action_id, __( 'action complete', 'easybook-add-ons' ) );
	}

	public function log_failed_action( $action_id, Exception $exception ) {
		$this->log( $action_id, sprintf( __( 'action failed: %s', 'easybook-add-ons' ), $exception->getMessage() ) );
	}

	public function log_timed_out_action( $action_id, $timeout ) {
		$this->log( $action_id, sprintf( __( 'action timed out after %s seconds', 'easybook-add-ons' ), $timeout ) );
	}

	public function log_unexpected_shutdown( $action_id, $error ) {
		if ( ! empty( $error ) ) {
			$this->log( $action_id, sprintf( __( 'unexpected shutdown: PHP Fatal error %s in %s on line %s', 'easybook-add-ons' ), $error['message'], $error['file'], $error['line'] ) );
		}
	}

	public function log_reset_action( $action_id ) {
		$this->log( $action_id, __( 'action reset', 'easybook-add-ons' ) );
	}

	public function log_ignored_action( $action_id ) {
		$this->log( $action_id, __( 'action ignored', 'easybook-add-ons' ) );
	}

	public function log_failed_fetch_action( $action_id ) {
		$this->log( $action_id, __( 'There was a failure fetching this action', 'easybook-add-ons' ) );
	}
}
