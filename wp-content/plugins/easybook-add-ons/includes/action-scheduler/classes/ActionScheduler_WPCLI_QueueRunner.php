...
class ActionScheduler_WPCLI_QueueRunner extends ActionScheduler_Abstract_QueueRunner {
	...
	public function __construct( ActionScheduler_Store $store = null, ActionScheduler_FatalErrorMonitor $monitor = null, ActionScheduler_QueueCleaner $cleaner = null ) {
		if ( ! ( defined( 'WP_CLI' ) && WP_CLI ) ) {
			...
			throw new Exception( sprintf( __( 'The %s class can only be run within WP CLI.', 'easybook-add-ons' ), __CLASS__ ) );
		}
	...
	public function setup( $batch_size, $hooks = array(), $group = '', $force = false ) {
		...
		if ( $too_many ) {
			if ( $force ) {
				WP_CLI::warning( __( 'There are too many concurrent batches, but the run is forced to continue.', 'easybook-add-ons' ) );
			} else {
				WP_CLI::error( __( 'There are too many concurrent batches.', 'easybook-add-ons' ) );
			}
		}
	...
	protected function setup_progress_bar() {
		...
		$this->progress_bar = \WP_CLI\Utils\make_progress_bar(
			sprintf( _n( 'Running %d action', 'Running %d actions', $count, 'easybook-add-ons' ), number_format_i18n( $count ) ),
			$count
		);
	}
	...
	public function run() {
		...
		foreach ( $this->actions as $action_id ) {
			...
			if ( ! in_array( $action_id, $this->store->find_actions_by_claim_id( $this->claim->get_id() ) ) ) {
				WP_CLI::warning( __( 'The claim has been lost. Aborting current batch.', 'easybook-add-ons' ) );
				break;
			}
	...
	public function before_execute( $action_id ) {
		...
		WP_CLI::log( sprintf( __( 'Started processing action %s', 'easybook-add-ons' ), $action_id ) );
	}
	...
	public function after_execute( $action_id, $action = null ) {
		...
		/* translators: %s refers to the action ID */
		WP_CLI::log( sprintf( __( 'Completed processing action %s with hook: %s', 'easybook-add-ons' ), $action_id, $action->get_hook() ) );
	}
	...
	public function action_failed( $action_id, $exception ) {
		WP_CLI::error(
			...
			sprintf( __( 'Error processing action %1$s: %2$s', 'easybook-add-ons' ), $action_id, $exception->getMessage() ),
			false
		);
	}
	...
	protected function stop_the_insanity( $sleep_time = 0 ) {
		if ( 0 < $sleep_time ) { 
			WP_CLI::warning( sprintf( 'Stopped the insanity for %d %s', $sleep_time, _n( 'second', 'seconds', $sleep_time, 'easybook-add-ons' ) ) );
			sleep( $sleep_time );
		}

		WP_CLI::warning( __( 'Attempting to reduce used memory...', 'easybook-add-ons' ) );
