...
class ActionScheduler_wpPostStore extends ActionScheduler_Store {
	...
	public function save_action( ActionScheduler_Action $action, DateTime $scheduled_date = NULL ){
		try {
			...
			throw new RuntimeException( sprintf( __('Error saving action: %s', 'easybook-add-ons'), $e->getMessage() ), 0 );
		}
	}
	...
	protected function save_post_array( $post_array ) {
		...
		if ( is_wp_error($post_id) || empty($post_id) ) {
			throw new RuntimeException(__('Unable to save action.', 'easybook-add-ons'));
		}
		return $post_id;
	}
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, array( 'select', 'count' ) ) ) {
			throw new InvalidArgumentException(__('Invalid schedule. Cannot save action.', 'easybook-add-ons'));
		}
	...
	public function cancel_action( $action_id ) {
		...
		if ( empty($post) || ($post->post_type != self::POST_TYPE) ) {
			throw new InvalidArgumentException(sprintf(__('Unidentified action %s', 'easybook-add-ons'), $action_id));
		}
	...
	protected function claim_actions( $claim_id, $limit, DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( $rows_affected === false ) {
			throw new RuntimeException( __( 'Unable to claim actions. Database error.', 'easybook-add-ons' ) );
		}
	...
	protected function get_actions_by_group( $group, $limit, DateTime $date ) {
		...
		if ( ! term_exists( $group, self::GROUP_TAXONOMY )) {
			throw new InvalidArgumentException( sprintf( __( 'The group "%s" does not exist.', 'easybook-add-ons' ), $group ) );
		}
	...
	public function release_claim( ActionScheduler_ActionClaim $claim ) {
		...
		if ( $result === false ) {
			throw new RuntimeException( sprintf( __('Unable to unlock claim %s. Database error.', 'easybook-add-ons'), $claim->get_id() ) );
		}
	}
	...
	public function unclaim_action( $action_id ) {
		...
		$result = $wpdb->query($sql);
		if ( $result === false ) {
			throw new RuntimeException( sprintf( __('Unable to unlock claim on action %s. Database error.', 'easybook-add-ons'), $action_id ) );
		}
	}
	...
	public function mark_failure( $action_id ) {
		...
		$result = $wpdb->query($sql);
		if ( $result === false ) {
			throw new RuntimeException( sprintf( __('Unable to mark failure on action %s. Database error.', 'easybook-add-ons'), $action_id ) );
		}
	}
	...
	public function get_status( $action_id ) {
		...
		if ( $status === null ) {
			throw new InvalidArgumentException( __( 'Invalid action ID. No status found.', 'easybook-add-ons' ) );
		}
