...
// Page ID
if(!function_exists('easybook_page_columns_head')){
    function easybook_page_columns_head($defaults) {
        $defaults['page_id'] = 'ID';
        return $defaults;
    }
}
if(!function_exists('easybook_page_columns_content')){
    // CUSTOM POSTS
    function easybook_page_columns_content($column_name, $post_ID) {
...
add_filter('manage_page_posts_columns', 'easybook_page_columns_head', 10);
add_action('manage_page_posts_custom_column', 'easybook_page_columns_content', 10, 2);

// Post ID
if(!function_exists('easybook_post_columns_head')){
    function easybook_post_columns_head($defaults) {
        $defaults['post_id'] = 'ID';
        return $defaults;
    }
}
if(!function_exists('easybook_post_columns_content')){
    // CUSTOM POSTS
    function easybook_post_columns_content($column_name, $post_ID) {
...
add_filter('manage_post_posts_columns', 'easybook_post_columns_head', 10);
add_action('manage_post_posts_custom_column', 'easybook_post_columns_content', 10, 2);
...
abstract class Esb_Class_CPT {
    ...
    protected function set_meta_option_default(){
        $this->meta_df_options = array(
            'title'                 => __( 'Default title', 'easybook-add-ons' ),
