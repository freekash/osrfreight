...
if ( ! empty( $listing_terms ) && ! is_wp_error( $listing_terms ) ) { ?>
    <div class="<?php echo esc_attr($css_class );?>">
        ...
        <?php 
        $item_width = explode(',',$items_width);
        // $item_width = array_filter($item_width);
        $key = 0;
        foreach ($listing_terms as $term) { 
            ...
            if(isset($item_width[$key])){
                switch ($item_width[$key]) {
                    case 'x2':
    ...
    <?php
        ...
        if($url != '') { 
            echo '<div class="view-all-cats"><a href="' . $url . '" ' . $target .' class="btn color-bg">'.$btn_name.'<i class="fa fa-angle-right"></i></a></div>'; 
        }else {
            echo '<div class="view-all-cats"><a href="#!" ' . $target .' class="btn color-bg">'.$btn_name.'<i class="fa fa-angle-right"></i></a></div>'; 
        }
    ?>
<?php } ?>
