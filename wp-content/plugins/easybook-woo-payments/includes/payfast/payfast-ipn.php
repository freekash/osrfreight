...
class CTH_Woo_Payments_PayFast_IPN extends CTH_Woo_Payments_PayFast_Response {
    ...
    protected function payment_status_complete( $order, $posted ) {
        ...
        if ( 'complete' === $posted['payment_status'] || 'trialing' === $posted['payment_status'] ) {
            ...
            $this->payment_complete( $order, ( ! empty( $posted['txn_id'] ) ? wc_clean( $posted['txn_id'] ) : '' ), __( 'IPN payment completed', 'easybook-woo-payments' ) );
            ...
            if ( 'authorization' === $posted['pending_reason'] ) {
                $this->payment_on_hold( $order, __( 'Payment authorized. Change payment status to processing or complete to capture funds.', 'easybook-woo-payments' ) );
            } else {
                /* translators: %s: pending reason. */
                $this->payment_on_hold( $order, sprintf( __( 'Payment pending (%s).', 'easybook-woo-payments' ), $posted['pending_reason'] ) );
            }
        }
    }
    ...
    protected function process_next_payment($order, $posted){
        ...
        // easybook_add_ons_active_membership($data, 'utc');
    ...
    protected function validate_currency( $order, $currency ) {
        if ( $order->get_currency() !== $currency ) {
            ...
            $order->update_status( 'on-hold', sprintf( __( 'Validation error: PayFast currencies do not match (code %s).', 'easybook-woo-payments' ), $currency ) );
            exit;
        }
    }
    ...
    protected function validate_amount( $order, $amount ) {
        if ( number_format( $order->get_total(), 2, '.', '' ) !== number_format( $amount, 2, '.', '' ) ) {
            ...
            $order->update_status( 'on-hold', sprintf( __( 'Validation error: PayFast amounts do not match (gross %s).', 'easybook-woo-payments' ), $amount ) );
            exit;
        }
    }
    ...
    protected function validate_receiver_email( $order, $receiver_email ) {
        if ( strcasecmp( trim( $receiver_email ), trim( $this->receiver_email ) ) !== 0 ) {
            ...
            $order->update_status( 'on-hold', sprintf( __( 'Validation error: PayFast IPN response from a different email address (%s).', 'easybook-woo-payments' ), $receiver_email ) );
            exit;
        }
    }
    ...
    protected function payment_status_paid_cancelled_order( $order, $posted ) {
        $this->send_ipn_email_notification(
            ...
            sprintf( __( 'Payment for cancelled order %s received', 'easybook-woo-payments' ), '<a class="link" href="' . esc_url( $order->get_edit_order_url() ) . '">' . $order->get_order_number() . '</a>' ),
            /* translators: %s: order ID. */
            sprintf( __( 'Order #%s has been marked paid by PayFast IPN, but was previously cancelled. Admin handling required.', 'easybook-woo-payments' ), $order->get_order_number() )
        );
    }
