...
class CTH_Woo_Payments_PayFast_Request {
	...
	public function get_request_url( $order, $sandbox = false ) {
        ...
        //     return str_replace("http://localhost:8888/easybook/", 'https://easybook2.cththemes.com/', $v);
	...
	protected function get_order_item_recurring($order){
		...
		foreach ( $order->get_items() as $item ) {
            ...
            if(!empty($trial_interval) && !empty($trial_period)){
                ...
                // after trial date
                $recurring_args['billing_date'] = easybook_add_ons_cal_next_date('now', $trial_period, $trial_interval, 'Y-m-d');
            }
            ...
            $recurring_args['frequency'] = easybook_addons_payfast_frequency( get_post_meta( $product_id , ESB_META_PREFIX.'interval', true ), get_post_meta( $product_id , ESB_META_PREFIX.'period', true ) ); // Subscription duration
   ...
   //          $recurring_args['item_number'] = $product_id;
	...
	protected function get_transaction_args( $order ) {
		return array_merge(
			array(
                ...
                'item_name'     => $this->get_order_item_names($order) ? $this->get_order_item_names($order) : __( 'Order', 'easybook-woo-payments' ),
	...
	protected function number_format( $price, $order ) {
        ...
        $price *= (float)easybook_addons_get_option('payfast_rate');
