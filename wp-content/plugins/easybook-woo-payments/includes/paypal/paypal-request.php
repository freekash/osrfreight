...
class CTH_Woo_Payments_Paypal_Request {
	...
	protected function get_order_item_recurring($order){
		...
		foreach ( $order->get_items() as $item ) {
			...
			$recurring_args['p3'] = easybook_add_ons_get_paypal_duration( get_post_meta( $product_id , ESB_META_PREFIX.'interval', true ), get_post_meta( $product_id , ESB_META_PREFIX.'period', true ) ); // Subscription duration
			$recurring_args['t3'] = easybook_add_ons_get_paypal_duration_unit( get_post_meta( $product_id , ESB_META_PREFIX.'period', true ) ); // Regular subscription units of duration.
			/*
            ...
            */
            ...
            if(!empty($trial_interval) && !empty($trial_period)){
                ...
                $recurring_args['p1'] = easybook_add_ons_get_paypal_duration( $trial_interval, $trial_period );
                $recurring_args['t1'] = easybook_add_ons_get_paypal_duration_unit( $trial_period );
            }
