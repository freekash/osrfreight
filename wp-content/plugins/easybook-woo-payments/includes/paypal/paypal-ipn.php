...
class CTH_Woo_Payments_Paypal_IPN extends CTH_Woo_Payments_Paypal_Response {
	...
	protected function validate_currency( $order, $currency ) {
		if ( $order->get_currency() !== $currency ) {
			...
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal currencies do not match (code %s).', 'easybook-woo-payments' ), $currency ) );
			exit;
		}
	}
	...
	protected function validate_amount( $order, $amount ) {
		if ( number_format( $order->get_total(), 2, '.', '' ) !== number_format( $amount, 2, '.', '' ) ) {
			...
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal amounts do not match (gross %s).', 'easybook-woo-payments' ), $amount ) );
			exit;
		}
	}
	...
	protected function validate_receiver_email( $order, $receiver_email ) {
		if ( strcasecmp( trim( $receiver_email ), trim( $this->receiver_email ) ) !== 0 ) {
			...
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal IPN response from a different email address (%s).', 'easybook-woo-payments' ), $receiver_email ) );
			exit;
		}
	}
	...
	protected function payment_status_paid_cancelled_order( $order, $posted ) {
		$this->send_ipn_email_notification(
			...
			sprintf( __( 'Order #%s has been marked paid by PayPal IPN, but was previously cancelled. Admin handling required.', 'easybook-woo-payments' ), $order->get_order_number() )
		);
	}
