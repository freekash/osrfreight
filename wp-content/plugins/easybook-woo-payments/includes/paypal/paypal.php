...
class CTH_Woo_Payments_Paypal extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->order_button_text = __( 'Proceed to PayPal', 'easybook-woo-payments' );
        $this->method_title      = __( 'EasyBook PayPal', 'easybook-woo-payments' );
        /* translators: %s: Link to WC system status page */
        $this->method_description = __( 'This is currently needed for selling RECURRING membership package. It redirects customers to PayPal to enter their payment information.', 'easybook-woo-payments' );
        // $this->supports           = array(
        //     'products',
        //     'refunds',
        // );
        ...
        if ( $this->testmode ) {
            ...
            $this->description .= ' ' . sprintf( __( 'SANDBOX ENABLED. You can use sandbox testing accounts only. See the <a href="%s">PayPal Sandbox Testing Guide</a> for more details.', 'easybook-woo-payments' ), 'https://developer.paypal.com/docs/classic/lifecycle/ug_sandbox/' );
    ...
    public function admin_options() {
        if ( $this->is_valid_for_use() ) {
            ...
            <div class="inline error">
                <p>
                    <strong><?php esc_html_e( 'Gateway disabled', 'easybook-woo-payments' ); ?></strong>: <?php esc_html_e( 'PayPal does not support your store currency.', 'easybook-woo-payments' ); ?>
                </p>
            </div>
            <?php
        }
    ...
    public function get_icon() {
        ...
        foreach ( $icon as $i ) {
            $icon_html .= '<img src="' . esc_attr( $i ) . '" alt="' . esc_attr__( 'PayPal acceptance mark', 'easybook-woo-payments' ) . '" />';
        }

        $icon_html .= sprintf( '<a href="%1$s" class="about_paypal" onclick="javascript:window.open(\'%1$s\',\'WIPaypal\',\'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1060, height=700\'); return false;">' . esc_attr__( 'What is PayPal?', 'easybook-woo-payments' ) . '</a>', esc_url( $this->get_icon_url( WC()->countries->get_base_country() ) ) );
