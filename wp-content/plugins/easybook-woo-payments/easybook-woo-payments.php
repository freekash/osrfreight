<?php 
/**
Plugin Name: EasyBook WooCommerce Payments
Plugin URI: https://easybook.cththemes.com
Description: A custom plugin for EasyBook - Hotel & Tour Booking WordPress Theme
Version: 1.1.7
Author: CTHthemes
Author URI: http://themeforest.net/user/cththemes
Text Domain: easybook-woo-payments
...
function easybook_woo_payments_missing_wc_notice() {
    /* translators: 1. URL link. */
    echo '<div class="error"><p><strong>' . sprintf( esc_html__( 'EasyBook WooCommerce Payments requires WooCommerce to be installed and active. You can download %s here.', 'easybook-woo-payments' ), '<a href="https://woocommerce.com/" target="_blank">WooCommerce</a>' ) . '</strong></p></div>';
}

function easybook_woo_payments_missing_adons() {
    /* translators: 1. URL link. */
    echo '<div class="error"><p><strong>' . esc_html__( 'EasyBook WooCommerce Payments requires EasyBook Add-Ons to be installed and active. You can install it from Appearance -> Install Plugins screen.', 'easybook-woo-payments' ) . '</strong></p></div>';
}

add_action( 'plugins_loaded', 'easybook_woo_payments_init' );

function easybook_woo_payments_init() {
    load_plugin_textdomain( 'easybook-woo-payments', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' );

    if ( ! class_exists( 'WooCommerce' ) ) {
        add_action( 'admin_notices', 'easybook_woo_payments_missing_wc_notice' );
        return;
    }

    if ( ! class_exists( 'EasyBook_Addons' ) ) {
        add_action( 'admin_notices', 'easybook_woo_payments_missing_adons' );
        return;
    }
