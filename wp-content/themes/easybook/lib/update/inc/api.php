...
class Api
{
    ...
    {
        _doing_it_wrong(__FUNCTION__, esc_html__('Cheatin&#8217; huh?', 'easybook'), '1.0.0');
    }
    ...
    public function request($url, $args = array())
    {
        ...
        if (empty($token)) {
            return new \WP_Error('api_token_error', __('An API token is required.', 'easybook'));
        }
        ...
        if (200 !== $response_code && !empty($response_message)) {
            ...
            return new \WP_Error($response_code, __('An unknown API error occurred.', 'easybook'));
        } else {
            $return = json_decode(wp_remote_retrieve_body($response), true);
            if (null === $return) {
                return new \WP_Error('api_error', __('An unknown API error occurred.', 'easybook'));
            }
            return $return;
        }
    }
